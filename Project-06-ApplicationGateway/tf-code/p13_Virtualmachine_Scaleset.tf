...
resource "azurerm_linux_virtual_machine_scale_set" "web_vmss01" {
  name = "${local.resource_name_prefix}-web-vmss01"
...
resource "azurerm_linux_virtual_machine_scale_set" "web_vmss02" {
  ...
  network_interface {
    ...
    ip_configuration {
      ...
      primary = true
      subnet_id = azurerm_subnet.websubnet.id 
      application_gateway_backend_address_pool_ids = [for pool in azurerm_application_gateway.web_ag.backend_address_pool : pool.id if pool.name == local.backend_address_pool_name_app2
]      
    }
  }
}